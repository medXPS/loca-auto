...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const conditions = [];
      if (carId)
        conditions.push(eq(schema.carExpensesTable.carId, parseInt(carId)));
      ...
      const expenses = await db
        ...
        .from(schema.carExpensesTable)
        .where(where)
        .orderBy(desc(schema.carExpensesTable.date))
        .limit(limitNum)
...
router.post(
  ...
  async (req, res) => {
    try {
      const { carId, type, amount, date, description, invoiceFileUrl } =
        req.body;
      const [expense] = await db
        .insert(schema.carExpensesTable)
        .values({
          carId,
          type,
...
router.delete(
  ...
  async (req, res) => {
    try {
      await db
        .delete(schema.carExpensesTable)
        .where(
          eq(schema.carExpensesTable.id, parseInt(String(req.params.id), 10)),
        );
      res.status(204).send();
    } catch (err) {
      req.log.error(err);
