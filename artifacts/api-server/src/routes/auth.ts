...
router.post("/resend-verification", async (req, res) => {
  try {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable during resend verification");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
...
router.post("/verify-email", async (req, res) => {
  try {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable during email verification");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
...
router.post("/login", async (req, res) => {
  try {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable during login");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
...
router.post("/verify-mfa", async (req, res) => {
  try {
    ...
    if (
      !user?.mfaCodeHash ||
      !user.mfaCodeExpiresAt ||
      user.mfaCodeExpiresAt.getTime() < Date.now()
    ) {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable during MFA verification");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
...
router.get("/me", authMiddleware, async (req, res) => {
  try {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable while reading current user");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
...
router.patch("/me", authMiddleware, async (req, res) => {
  try {
    ...
    if (isDatabaseUnavailableError(err)) {
      req.log.warn(err, "Database unavailable while updating current user");
      res.status(503).json({
        error: "Service temporairement indisponible",
      });
      return;
